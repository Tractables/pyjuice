...
def test_external_categorical_dist_sample_with_mask():
    ...
    inputs = torch.zeros([128, 2], dtype = torch.long, device = device)
    inputs[64:,0] = 1
    inputs[:64,0] = 0

    external_categorical_value_mask = torch.zeros([128, 2], dtype = torch.bool, device = device)
    external_categorical_value_mask[:,0] = True

    samples = juice.queries.sample(pc, num_samples = 128, external_categorical_logps = external_categorical_logps,
                                   inputs = inputs, external_categorical_value_mask = external_categorical_value_mask)

    assert (samples[:,1] == 1).float().mean() > 0.7
    assert torch.all(samples[:64,0] == 0)
