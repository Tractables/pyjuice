...
@triton.jit
def my_kernel(aaa, bbb, ccc, node_flows, element_flows, node_mars, element_mars, params, 
            chids, parids_start, parids_increment, parpids_start, parpids_increment, 
            local_ids, batch_size: tl.constexpr, partial_eval: tl.constexpr, ptr_inc_step: tl.constexpr, 
            BLOCK_B: tl.constexpr, TILE_SIZE_K: tl.constexpr, K_NUM_TILES: tl.constexpr, TILE_SIZE_M: tl.constexpr, 
            GROUP_SIZE_M: tl.constexpr, GROUP_SIZE_K: tl.constexpr):
    ...
    for k in range(0, 1):
        ...
        offs_aaa = pid_m * (TILE_SIZE_K * batch_size) + tl.arange(0, TILE_SIZE_K)[:,None] * batch_size + offs_batch[None,:]
        tl.store(aaa + offs_aaa, n_fdm_sub, mask = mask_batch[None,:])

        partial_flows = tl.dot(epars, n_fdm_sub)
        # partial_flows = tl.sum(epars[:,:,None] * n_fdm_sub[None,:,:], axis = 1)

        offs_bbb = pid_m * (TILE_SIZE_M * batch_size) + tl.arange(0, TILE_SIZE_M)[:,None] * batch_size + offs_batch[None,:]
        tl.store(bbb + offs_bbb, partial_flows, mask = mask_batch[None,:])

        offs_ccc = pid_m * batch_size + offs_batch
        tl.store(ccc + offs_ccc, log_n_fdm_max, mask = mask_batch)
...
def main():
    ...
    element_flows = torch.from_numpy(data["element_flows"]).to(device)
    ...
    aaa = torch.zeros([grid[1], TILE_SIZE_K, batch_size]).cuda()
    bbb = torch.zeros([grid[1], TILE_SIZE_M, batch_size]).cuda()
    ccc = torch.zeros([grid[1], batch_size]).cuda()
