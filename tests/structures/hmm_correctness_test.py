...
def test_hmm_forward_backward_with_generalized_em():
        ...
        for ns in root_ns(reverse = True):
            if ns == root_ns:
                ...
                for i, cs in enumerate(ns.chs):
                    ...
                    eflows = nflows * (params.log() * alpha).exp().permute(1, 0) * ((emars - nmars) * alpha).exp()
                    pflows = (nflows * params.permute(1, 0) * (emars - nmars).exp()).sum(dim = 1)
                ...
                for i, cs in enumerate(ns.chs):
                    ...
                    log_n_fdm = nflows.log() - nmars * alpha
                    log_n_fdm_max = log_n_fdm.max(dim = 0).values
                    n_fdm_sub = (log_n_fdm - log_n_fdm_max[None,:]).exp()

                    eflows = torch.matmul((params.log() * alpha).exp().permute(1, 0), n_fdm_sub) * (emars * alpha + log_n_fdm_max[None,:]).exp()
