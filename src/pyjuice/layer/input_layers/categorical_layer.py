...
class CategoricalLayer(InputLayer, nn.Module):
    ...
    def _normalize_parameters(self, params, pseudocount: float = 0.0):
        if self.node_ids.is_cuda:
            ...
            grid1 = lambda meta: (triton.cdiv(self.num_params, meta['BLOCK_SIZE']),)
            grid2 = lambda meta: (triton.cdiv(self.num_params, meta['BLOCK_SIZE']),)

            self._cum_params_kernel[grid1](params, cum_params, self.node_ids, self.num_params, BLOCK_SIZE = 1024)
            self._norm_params_kernel[grid2](params, cum_params, self.node_ids, self.node_nchs, self.num_params, 
                                            pseudocount, BLOCK_SIZE = 1024)

        else:
            with torch.no_grad():
                param_ids = torch.arange(0, self.num_params, dtype = torch.long, device = params.device)

                cum_matrix1 = torch.sparse_coo_tensor(
                    torch.stack((self.node_ids, param_ids), dim = 0), 
                    params.view(-1), (self.param_ends.size(0), self.num_params)
                )
                node_buffer = torch.sparse.mm(cum_matrix1, torch.ones([self.num_params, 1], dtype = torch.float32, device = params.device))

                node_buffer.reciprocal_()

                cum_matrix2 = torch.sparse_coo_tensor(
                    torch.stack((param_ids, self.node_ids), dim = 0), 
                    params.view(-1), (self.num_params, self.param_ends.size(0))
                )
