    ...
    def update_ns(ns: CircuitNodes, ns_chs: Sequence[CircuitNodes]):
        ...
        if ns.isinput():
            ...
            if torch.all(edge_ids[:,1:,:] - edge_ids[:,:-1,:]) == 1:
                ...
                # Sparse mode
                edge_ids = (edge_ids.reshape(ns.group_size, ns.num_chs)[:,None,:] * ns.group_size + \
            ...
            new_cs_group_size = ns2group_size[ns.chs[0]]
            cs_group_mul_size = new_cs_group_size // ns.chs[0].group_size

            new_num_cgroups = old_num_cgroups // cs_group_mul_size

            edge_ids = ns.edge_ids.clone()
            grid_edge_ids = torch.zeros([old_num_ngroups, old_num_cgroups], dtype = torch.bool)
            grid_edge_ids[edge_ids[0,:],edge_ids[1,:]] = True

            grid_edge_ids = grid_edge_ids.reshape(new_num_ngroups, group_mul_size, new_num_cgroups, cs_group_mul_size)
            ...
            if not ns.is_tied():
                ...
                if params is not None:
                    ...
                    new_params = torch.zeros([new_edge_ids.size(1), new_group_size, new_cs_group_size])
                    for par_group_id in range(new_edge_ids.size(1)):
                        nsid = new_edge_ids[0,i] * group_mul_size
                        neid = nsid + group_mul_size
                        csid = new_edge_ids[1,i] * cs_group_mul_size
                        ceid = csid + cs_group_mul_size

                        blk_ids = torch.where((nsid <= edge_ids[0,:] < neid) & (csid <= edge_ids[1,:] < ceid))
                        for blk_id in blk_ids:
                            nid0, nid1 = (edge_ids[0,:] - nsid) * ns.group_size, (edge_ids[0,:] - nsid + 1) * ns.group_size
                            cid0, cid1 = (edge_ids[1,:] - csid) * ns.chs[0].group_size, (edge_ids[1,:] - csid + 1) * ns.chs[0].group_size
