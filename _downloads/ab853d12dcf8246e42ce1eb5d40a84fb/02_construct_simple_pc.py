...
# the :math:`i`th product node block connects to the :code:`edge_ids[i,j]`th node block in the :math:`j`th child node vector (assume we always count from 0).
...
# Every size-2 column vector :math:`[m, n]^T` in :code:`edge_ids` indicates the existance of fully-connected edges between the 
# :math:`m`th sum node block and the :math:`n`th product node block.
