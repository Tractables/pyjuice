...
# The goal of this tutorial is to get you familiar with these three functions.
# 
...
# %%
# The above line defines :code:`num_node_blocks * block_size = 64` input nodes on variable ID :code:`var = 0` featuring Categorical distributions with 4 categories (i.e., :code:`dists.Categorical(num_cats = 4)`). The set of input distributions are defined under :code:`pyjuice.nodes.distributions`.
#
...
# However, although using larger block sizes when defining input node vectors do not have any negative effects, it poses restrictions on the edge connection pattern as we shall proceed to show.
# 
# Note that :code:`block_size` has to be a power of 2.
# 
# Let us define another input node vector and a product node vector over them.
...
# 
# We assume the node vectors fed to :code:`pyjuice.multiply` have the same :code:`block_size`. And the block size of the output product node vector is also the same with that of the inputs.
...
# 
# Finally, when we do not provide the :code:`edge_ids`, PyJuice assumes it to be the following:
